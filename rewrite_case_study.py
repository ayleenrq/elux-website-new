        ...
        <section className="min-h-[80vh] bg-[#0a0a0a] text-white pt-40 px-8 flex flex-col items-center text-center relative overflow-hidden border-0 shadow-none">
            <div className="max-w-7xl mx-auto w-full flex flex-col items-center border-0 shadow-none z-10 relative">
                ...
                <p className="max-w-3xl text-gray-400 text-2xl mt-8 font-medium leading-relaxed border-0 shadow-none mb-32">
                    Simplifying complex data into intuitive user flows and progressive indicators for {project.title}.
                </p>
            </div>
...
function TheBrief({ project }: { project: any }) {
    return (
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-16 max-w-7xl mx-auto border-0 shadow-none">
                ...
                <div className="md:col-span-8 flex flex-col border-0 shadow-none">
                    <p className="text-2xl text-gray-600 leading-relaxed font-normal border-0 shadow-none mb-8">
                        The legacy platform for {project.title} suffered from immense cognitive overload. Users loved the power, but stalled during activation. The core challenge was distilling complex models into actionable, progressive insights—without dulling the feature set.
                    </p>
...
function PragmaticOutcomes({ project }: { project: any }) {
    return (
            ...
            <div className="max-w-7xl mx-auto border-0 shadow-none">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-16 border-0 shadow-none">
                    <div className="border-l border-gray-200 pl-8 shadow-none">
                        <h3 className="text-6xl font-semibold tracking-tighter text-gray-900 mb-4 border-0 shadow-none">{project.metric.split(' ')[0]}</h3>
                        <p className="text-xl text-gray-600 font-medium border-0 shadow-none">{project.metric.split(' ').slice(1).join(' ')} within first 7 days.</p>
                    </div>
                    <div className="border-l border-gray-200 pl-8 shadow-none">
...
function NextProject({ nextProject }: { nextProject: any }) {
    return (
        <a href={`#case-study-detail-${nextProject.id}`} onClick={() => { window.location.hash = `#case-study-detail-${nextProject.id}`; }} className="block w-full bg-[#0a0a0a] text-white py-40 px-8 flex flex-col items-center justify-center cursor-pointer hover:bg-[#111111] transition-colors border-0 shadow-none group">
...
export default function CaseStudyDetail() {
    const [project, setProject] = useState<any>(caseStudies[0]);
    const [nextProject, setNextProject] = useState<any>(caseStudies[1]);

    useEffect(() => {
        const handleHash = () => {
            const hashParts = window.location.hash.split('-');
            const id = hashParts[hashParts.length - 1];
            const foundIndex = caseStudies.findIndex((p: any) => p.id.toString() === id);
            
            if (foundIndex !== -1) {
                setProject(caseStudies[foundIndex]);
                setNextProject(caseStudies[(foundIndex + 1) % caseStudies.length]);
            } else {
                setProject(caseStudies[0]);
                setNextProject(caseStudies[1]);
            }
            window.scrollTo(0, 0);
        };
    ...
    return (
        <div className="font-sans antialiased text-gray-900 bg-white border-0 shadow-none">
            <main className="border-0 shadow-none">
                ...
                <TheBrief project={project} />
                <DesignSystemAndBento />
                <PragmaticOutcomes project={project} />
